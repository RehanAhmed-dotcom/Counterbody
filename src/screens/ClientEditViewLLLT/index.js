...
const ClientEditViewLLLT = ({navigation, route}) => {
  ...
  const checkValueschec = (id, ansid) => {
    checkuncheck(id) == 'yes'
      ? additems(id, '-1', ansid)
      : additems(id, 'yes', ansid);
  };
        ...
        <View style={{flexDirection: 'row'}}>
          <View style={{flexDirection: 'row', alignItems: 'center'}}>
            <Icon1
              ...
              size={20}
              color={'black'}
              onPress={() => checkValueschec(item.id, item.answer_id)}
            />
...
export default ClientEditViewLLLT;
